...
const Patisserie = () => {
  ...
  // Fetch only desserts from the existing menu data, or define them here
  const patisserieData = [
    { id: 4, category: t('menu.cat_desserts'), name: t('menu.item4.name'), price: 14, description: t('menu.item4.desc') },
    // You can add more pastries here in the future
  ]
              ...
              >
                <Card3D intensity={10} style={{ 
                  ...
                  <div style={{ height: '220px', position: 'relative', overflow: 'hidden', backgroundColor: 'var(--navy)' }}>
                    ...
                    {/* Category Badge */}
                    <div style={{ position: 'absolute', top: '10px', right: '10px', backgroundColor: 'var(--gold)', color: 'var(--navy)', fontSize: '0.65rem', fontWeight: 'bold', padding: '3px 8px', borderRadius: '20px', textTransform: 'uppercase', letterSpacing: '0.5px', zIndex: 3 }}>
                      {item.category}
                    </div>

                    {/* Steam overlay (optional, generally pastries aren't steaming, but kept for consistency if ever hot desserts added) */}
                     {/* 
                     <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(0,31,63,0.8), transparent 80%)', zIndex: 1 }}>
                       <SteamOverlay count={2} height="30px" opacity={0.3} />
                     </div> 
                     */}
                  </div>
